...
def ensure_session():
    ...
    # Usar setdefault para no resetear el peso si ya existe
    ...
    if len(d_fit) < 3:
        ...
        if st.button("AJUSTAR"):
            ...
            st.session_state.models_fit = {
                ...
                'Doble Weibull': {'FO':FO_dw, 'params':dw_params}
            }
        ...
        if st.session_state.models_fit:
            ...
            fo_tbl = pd.DataFrame([
                {'Modelo':k, 'F.O.':v['FO'], 'Parametros':str(v['params'])} for k,v in fits.items()
            ])
            st.table(fo_tbl)
...
def page_6():
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if models:
            models_df = pd.DataFrame([
                {'Modelo':k, 'F.O.':v['FO'], 'Parametros':str(v['params'])} for k,v in models.items()
            ])
            models_df.to_excel(writer, sheet_name='ModelosAjuste', index=False)
            
    output.seek(0)
    b64 = base64.b64encode(output.read()).decode()
    href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="analisis_granulometrico.xlsx">Descargar Excel (analisis_granulometrico.xlsx)</a>'
    ...
    if st.button("GUARDAR (generar QR de descarga)"):
        ...
        st.image(qr.get_image(), caption="Escanea el QR para descargar el archivo.")
        st.success("Archivo generado en código QR para descarga.")
