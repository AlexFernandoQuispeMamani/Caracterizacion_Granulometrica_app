...
def page_5():
    ...
    if st.button("AJUSTAR"):
        # ------------------ GGS ------------------
        try:
            # Valor inicial: m=1, Dm = max(d) cercano al tamaño máximo experimental
            x0 = [1.0, max(d)]
            def f_ggs(params):
                m, Dm = params
                ypred = GGS_model(d, m, Dm)
                return np.sum((y_exp - ypred)**2)
            res1 = minimize(f_ggs, x0, bounds=[(0.01,10),(max(d)*0.5, max(d)*1.5)])
        ...
        # ------------------ RRSB ------------------
        try:
            # Valor inicial: m=1, l = d donde %F ~ 63.2
            interp_func = interp1d(y_exp, d, fill_value="extrapolate")
            l0 = float(interp_func(63.2))
            x0 = [1.0, l0]
            def f_rrsb(params):
                m, l = params
                ypred = RRSB_model(d, m, l)
                return np.sum((y_exp - ypred)**2)
            res2 = minimize(f_rrsb, x0, bounds=[(0.01,10),(min(d)*0.5,max(d)*2)])
    ...
    if st.session_state.get("models_fit", None):
        ...
        for i in range(len(d_inferior)):
            ...
            if not np.isnan(fits['GGS']['params']).any():
                ...
                row["%F(d)m_GGS"] = min(100.0, y_g)
                row["ε²_GGS"] = f"{((ye - min(100.0, y_g))/ye)**2:.2e}" if ye !=0 else np.nan
            else:
            ...
            if not np.isnan(fits['RRSB']['params']).any():
                ...
                row["%F(d)m_RRSB"] = min(100.0, y_r)
                row["ε²_RRSB"] = f"{((ye - min(100.0, y_r))/ye)**2:.2e}" if ye !=0 else np.nan
            else:
            ...
            if not np.isnan(fits['DoubleWeibull']['params']).any():
                ...
                row["%F(d)m_DW"] = min(100.0, y_dw)
                row["ε²_DW"] = f"{((ye - min(100.0, y_dw))/ye)**2:.2e}" if ye !=0 else np.nan
            else:
    ...
    if st.session_state.models_fit:
        ...
        st.table(fo_tbl.style.format({'F.O.':'{:.2e}'}))
        best = min(fits.items(), key=lambda x: x[1]['FO'])
        best_model_name = best[0]
        st.markdown(f"**Mejor modelo:** {best_model_name} con F.O. = {best[1]['FO']:.2e}")
    ...
    if st.session_state.models_fit:
        ...
        y_ggs = np.clip(GGS_model(dd, *fits['GGS']['params']), None, 100.0) if not np.isnan(fits['GGS']['params']).any() else None
        y_rrsb = np.clip(RRSB_model(dd, *fits['RRSB']['params']), None, 100.0) if not np.isnan(fits['RRSB']['params']).any() else None
        y_dw = np.clip(double_weibull(dd, *fits['DoubleWeibull']['params']), None, 100.0) if not np.isnan(fits['DoubleWeibull']['params']).any() else None
        ...
        fig.patch.set_facecolor('#d3d3d3')  # gris suave afuera del área del gráfico
        ax.set_facecolor('white')
        ...
        if graf_option == "Comparación de perfiles":
            ...
            if y_ggs is not None: ax.plot(dd, y_ggs, '-', color='k', linewidth=0.9, label='GGS')
            if y_rrsb is not None: ax.plot(dd, y_rrsb, '--', color='k', linewidth=0.9, label='RRSB')
            if y_dw is not None: ax.plot(dd, y_dw, ':', color='k', linewidth=0.9, label='DW')
            ax.set_title("Comparación de perfiles"); ax.set_xlabel("Tamaño (µm)"); ax.set_ylabel("%F(d)"); ax.set_ylim(0,100)
            ax.grid(True, ls='--', alpha=0.5); ax.legend()

        elif graf_option == "Diagrama GGS (log-log)":
            ax.set_xscale('log'); ax.set_yscale('log')
            ax.plot(xdata, np.clip(ydata,1e-6,100), 'x', markersize=6, markeredgewidth=1, markeredgecolor='k', label='Experimental')
            if y_ggs is not None: ax.plot(dd, np.clip(y_ggs,1e-6,100), '-', color='k', linewidth=0.8, label='GGS')
            ax.set_title("Diagrama GGS (log-log)"); ax.set_xlabel("Tamaño (µm) [log]"); ax.set_ylabel("%F(d) [log]")
            ax.grid(True, which='both', ls='--', alpha=0.5); ax.legend()

        elif graf_option == "Diagrama RRSB (log-x, transform y)":
            ax.set_xscale('log'); ax.set_title("Diagrama RRSB"); ax.set_xlabel("Tamaño (µm) [log]"); ax.set_ylabel("Log[ ln(1/(1-(%F/100))) ]")
            def transform_rrsb(y_percent):
                y = np.clip(y_percent,1e-8,99.9999)
                return np.log(np.log(1/(1 - y/100)))
            ydata_trans = transform_rrsb(ydata)
            ax.plot(xdata, ydata_trans, 'x', markersize=6, markeredgewidth=1, markeredgecolor='k', label='Experimental')
            if y_rrsb is not None: ax.plot(dd, transform_rrsb(y_rrsb), '-', color='k', linewidth=0.9, label='RRSB')
            ax.grid(True, ls='--', alpha=0.5); ax.legend()
            try:
                f_inv = interp1d(y_rrsb, dd, fill_value="extrapolate", bounds_error=False)
                d63 = float(f_inv(63.2))
                st.info(f"d_63.2 = {d63:.4f} µm")
            except: pass

        elif graf_option == "Diagrama DW (decimal)":
            ax.plot(xdata, ydata, **exp_marker_kwargs, label='Experimental')
            if y_dw is not None: ax.plot(dd, y_dw, '-', color='k', linewidth=0.9, label='DW')
            ax.set_title("Diagrama DW"); ax.set_xlabel("Tamaño (µm)"); ax.set_ylabel("%F(d)"); ax.set_ylim(0,100)
            ax.grid(True, ls='--', alpha=0.5); ax.legend()
